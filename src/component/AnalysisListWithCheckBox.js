...
function AnalysisListWithCheckBox({
    ...
    function handleConditionChange(e) {
        ...
        const newFilterList = handleListChange(newFilterCondition, list);
        setFilterCondition(newFilterCondition);
        if (newFilterList.length === 0) {
            console.log(
                `沒有符合篩選結果,清除原本篩選條件,按最後一次需求篩選資料`,
            );
            const newFilterConditionRule = filterConditionRule;
            newFilterConditionRule[name] = value;
            setFilterCondition(newFilterConditionRule);
    ...
    function handleImportProduct(e) {
        const dataId = e.id;
        const filterData = processingData.filter(
            element => element.id !== dataId,
        );
        if (filterData.length === processingData.length) {
